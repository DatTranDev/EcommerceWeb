...
-- triiger cho shopoder
-- Từ Đang chuẩn bị (ID 1) sang Đang vận chuyển (ID 2)
DELIMITER $$

CREATE TRIGGER reduce_stock_on_shipping
AFTER UPDATE ON ShopOrder
FOR EACH ROW
BEGIN
    IF OLD.OrderStatusID = 1 AND NEW.OrderStatusID = 2 THEN
        UPDATE ProductItem
        JOIN OrderLine ON ProductItem.ID = OrderLine.ProductItemID
        SET ProductItem.QuantityInStock = ProductItem.QuantityInStock - OrderLine.Quantity
        WHERE OrderLine.OrderID = NEW.ID;
...
-- Từ Đang vận chuyển (ID 2) sang Giao hàng thất bại (ID 4)

DELIMITER $$

CREATE TRIGGER restore_stock_on_failed_delivery
AFTER UPDATE ON ShopOrder
FOR EACH ROW
BEGIN
    IF OLD.OrderStatusID = 2 AND NEW.OrderStatusID = 4 THEN
        UPDATE ProductItem
        JOIN OrderLine ON ProductItem.ID = OrderLine.ProductItemID
        SET ProductItem.QuantityInStock = ProductItem.QuantityInStock + OrderLine.Quantity
        WHERE OrderLine.OrderID = NEW.ID;
...
-- trigger tự sinh bảng giỏ hàng cho user
DELIMITER $$

CREATE TRIGGER after_insert_user
AFTER INSERT ON SiteUser
FOR EACH ROW
BEGIN
...
INSERT INTO OrderStatus (Status, IsDeleted) VALUES ('Đã hủy', 0);
...
insert into SiteUser(DisplayName, Email, PhoneNumber, PassWord, Gender, Role)
    value ("Dat Tran", "neban0444@gmail.com", "0123456789", "admin", "Nam", "admin");
insert into SiteUser(DisplayName, Email, PhoneNumber, PassWord, Gender)
    value ("Nhan Tran", "user@gmail.com", "0123456789", "user", "Nữ");
...
insert into paymentMethod(DisplayName)values('Thanh toán khi nhận hàng');
insert into paymentMethod(DisplayName,isdeleted)values('Thanh toán trực tuyến',1);


insert into shippingmethod(displayname,price,isdeleted) values('Giao hàng nhanh',50000,0);
insert into shippingmethod(displayname,price,isdeleted) values('Giao hàng tiết kiệm',30000,0);
insert into shippingmethod(displayname,price,isdeleted) values('Giao hàng hỏa tốc',80000,1);

insert into address (value)values('Đông Kinh, Đông Hưng,Thái Bình');
insert into address (value)values('Thủ Đức, Thành Phồ Hồ Chí Minh');
insert into userAddress(userid,addressid,isdefault,isdeleted) values(2,1,0,0);
insert into userAddress(userid,addressid,isdefault,isdeleted) values(2,2,0,0);
